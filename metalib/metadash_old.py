...
def save_and_retrieve_historical_deals(new_merged_deals):
    if os.path.exists("../../historical_merged_deals.pkl"):
        print("Loading historical deals...")
        old_merged_deals = pd.read_pickle("../../historical_merged_deals.pkl")
    else:
        print("No historical deals found, creating new file...")
...
def main():
    ...
    account_size = st.sidebar.number_input(
        "Account Size ($)", value=100000, min_value=10000, step=1000
    )
    ...
    if st.sidebar.button("Fetch Trading Data"):
        with st.spinner("Fetching data from MT5..."):
            ...
            if len(history_orders) > 0:
                st.write(
                    f"history_orders_get({from_date}, {to_date}) = {len(history_orders)}"
                )
                ...
                with main_container:
                    ...
                    with tab1:
                        ...
                        df_deals_opens = df_deals[
                            df_deals["comment"].str.contains("meta")
                        ]
                        ...
                        merged_deals["time_open"] = pd.to_datetime(
                            merged_deals["time_open"], unit="s"
                        )
                        ...
                        daily_profit = (
                            overall_daily.groupby("date")["profit"].sum().reset_index()
                        )
                        ...
                        plot_data["strategy_symbol"] = (
                            plot_data["comment_open"] + " - " + plot_data["symbol_open"]
                        )
                    ...
                    with tab2:
                        ...
                        if "time_close" in merged_deals.columns:
                            ...
                            for symbol in merged_deals["symbol_open"].unique():
                                symbol_data = merged_deals[
                                    merged_deals["symbol_open"] == symbol
                                ]
                        ...
                        filtered_deals = merged_deals[
                            merged_deals["comment_open"] == strategy_choice
                        ].copy()
                    ...
                    with tab3:
                        ...
                        merged_deals["total_profit"] = (
                            merged_deals["profit_open"] + merged_deals["profit_close"]
                        )
                        merged_deals_sorted = merged_deals.sort_values("time_open")
                        merged_deals_sorted["cumulative_profit"] = merged_deals_sorted[
                            "total_profit"
                        ].cumsum()
                        ...
                        equity["drawdown"] = (
                            equity["equity"] / equity["running_max"] - 1
                        ) * 100
                        ...
                        for symbol in symbols:
                            symbol_data = merged_deals_sorted[
                                merged_deals_sorted["symbol_open"] == symbol
                            ]
                            symbol_data = symbol_data.sort_values("time_open")
                            symbol_data["cumulative_profit"] = symbol_data[
                                "total_profit"
                            ].cumsum()
                        ...
                        for symbol in symbols:
                            ...
                            scatter_fig.add_trace(
                                go.Scatter(
                                    ...
                                    name=symbol,
                                    marker=dict(
                                        size=np.abs(symbol_data["total_profit"])
                                        / max(
                                            1, np.abs(symbol_data["total_profit"]).max()
                                        )
                                        * 20
                                        + 5
                                    ),
                                    text=symbol_data[
                                        "comment_open"
                                    ],  # Using comment_open for hover info
                                )
                            )
                        ...
                        merged_deals_sorted["win"] = (
                            merged_deals_sorted["total_profit"] > 0
                        )

                        # Create a new column that identifies when the win/loss streak changes
                        merged_deals_sorted["streak_change"] = merged_deals_sorted[
                            "win"
                        ].ne(merged_deals_sorted["win"].shift())

                        # Assign a streak ID to each streak
                        merged_deals_sorted["streak_id"] = merged_deals_sorted[
                            "streak_change"
                        ].cumsum()
                        ...
                        streak_fig.add_trace(
                            go.Bar(
                                ...
                                customdata=np.stack(
                                    (
                                        win_streaks["symbol_open"],
                                        win_streaks["comment_open"],
                                    ),
                                    axis=1,
                                ),
                            )
                        ...
                        streak_fig.add_trace(
                            go.Bar(
                                ...
                                customdata=np.stack(
                                    (
                                        loss_streaks["symbol_open"],
                                        loss_streaks["comment_open"],
                                    ),
                                    axis=1,
                                ),
                            )
                        ...
                        strategy_perf = (
                            merged_deals.groupby("comment_open")["total_profit"]
                            .agg(["sum", "mean", "count", "std"])
                            .reset_index()
                        )
                        strategy_perf.columns = [
                            "Strategy",
                            "Total Profit",
                            "Average Profit",
                            "Number of Trades",
                    ...
                    with tab4:
                        ...
                        instance_deals = merged_deals.loc[
                            merged_deals["comment_open"] == selected_instance
                        ]
                        instance_deals = instance_deals[
                            [
                                "symbol_open",
                                "time_open",
                                "time_close",
