...
class MetaGO(MetaStrategy):
    def __init__(
        ...
        logging.basicConfig(
            filename=f"../logs/{self.tag}.log", encoding="utf-8", level=logging.DEBUG
        )
    ...
    def fit(self, data=None):
        ...
        next_returns = (
            hist_next_returns - hist_next_returns.mean()
        ) / hist_next_returns.std()
        ...
        aligned_close, aligned_true_open_monthly = close.align(
            true_open_monthly, join="inner"
        )
        ...
        mask_uptrend_below_yearly = (aligned_uptrend > 0) & (
            aligned_close < aligned_true_open_monthly
        )
        mask_downtrend_above_yearly = (align_downtrend > 0) & (
            aligned_close > aligned_true_open_monthly
        )
        ...
        print(
            f"{self.tag}::: DecisionTree Model trained from {X.index[0]} to {X.index[-1]}."
        )

        # Add line to log file to signal training completion
        self.logger.info(
            f"DecisionTree Model trained from {X.index[0]} to {X.index[-1]}."
        )
