...
class MetaFVG(MetaStrategy):
    ...
    def __init__(self, symbols, timeframe, size_position, tag, limit_number_position=1):
        """
    ...
    def detect_fvg_momentum_tres_strong(
        self, ohlc_df: pd.DataFrame, direction
    ) -> List[Dict]:
        """
        ...
        for i in range(len(ohlc_df) - 2):
            ...
            if direction:
                if candle_i["high"] < candle_i2["low"]:
                    ...
                    if (
                        candle_i1["close"] > candle_i1["open"]  # Bullish candle
                        and momentum_cond
                    ):  # Strong body ratio
                ...
                if candle_i["low"] > candle_i2["high"]:
                    if (
                        candle_i1["open"] > candle_i1["close"]  # Bearish candle
                        and momentum_cond
                    ):  # Strong body ratio
    ...
    def check_conditions(self):
        ...
        digits = (
            symbol_info.digits + 1
        )  # Use symbol_info.digits for proper rounding + add one because its after the decimal
        ...
        # Proper rounding
        tp, sl, entry = (
            round(self.tp, digits),
            round(self.sl, digits),
            round(self.entry, digits),
        )

        if self.state == 1:
            print(
                f"{self.tag}::    Long setup is valid - SL: ${sl}, ENTRY : ${entry},TP : ${tp}, VOLUME: {self.volume}"
            )
            self.execute(
                symbol=self.symbols[0],
    ...
    def signals(self):
        ...
        if current_open_position >= self.limit_number_position:
            print(
                f"{self.tag}::    Maximum open position reached, number of positions : {current_open_position}"
            )
            return
        ...
        if (
            len(self.filt_bearish_htf_fvg_patterns) == 0
            and len(self.filt_bullish_htf_fvg_patterns) == 0
        ):
        ...
        in_bullish_htf_fvg = bullish_htf_fvgs.apply(
            lambda x: (last_price < x["gap_low"]) and (last_price > x["gap_high"]),
            axis=1,
        )

        bearish_htf_fvgs = self.filt_bearish_htf_fvg_patterns
        in_bearish_htf_fvg = bearish_htf_fvgs.apply(
            lambda x: (last_price < x["gap_low"]) and (last_price > x["gap_high"]),
            axis=1,
        )
        ...
        if not in_bullish_htf_fvg.any() and not in_bearish_htf_fvg.any():
            print(
                f"{self.tag}::    Price not in any Bullish OR Bearish FVG H4, no action required"
            )
            return
        else:
            if in_bearish_htf_fvg.any() and in_bullish_htf_fvg.any():
                last_fvg_bull = in_bullish_htf_fvg[in_bullish_htf_fvg].iloc[-1][
                    "timestamp"
                ]
                last_fvg_bear = in_bearish_htf_fvg[in_bearish_htf_fvg].iloc[-1][
                    "timestamp"
                ]
                direction = 1 if last_fvg_bull > last_fvg_bear else 0
                print(
                    f"{self.tag}::    Price in both bearish and bullish FVG H4, direction: {direction}"
                )
            ...
            if in_bullish_htf_fvg.any():
                print(
                    f"{self.tag}::    Price in Bullish FVG H4, looking for long setups"
                )
                direction = 1
            elif in_bearish_htf_fvg.any():
                print(
                    f"{self.tag}::    Price in Bearish FVG H4, looking for short setups"
                )
                direction = 0

        momentum_fvg_patterns = self.detect_fvg_momentum_tres_strong(
            ohlc_ltf.iloc[-4:-1], direction
        )
        ...
        if (current_momentum["direction"] != "bullish") == bool(direction):
            print(
                f"{self.tag}::    Price not in the correct direction for FVG H4, no action required"
            )
            return
    ...
    def retrieve_fvg_crosses(self, fvg_pattern, price_ts, fill_pct):
        ...
        fvg_level = (
            fill_pct * fvg_pattern["gap_low"] + (1 - fill_pct) * fvg_pattern["gap_high"]
        )
    ...
    def fit(self):
        print(
            f"{self.tag}::     Starting the FVG Detection pelo!!"
        ...
        ohlc_resampled_df = ohlc.resample("4h").agg(
            {"open": "first", "high": "max", "low": "min", "close": "last"},
            label="right",
            closed="right",
        )

        bullish_htf_fvg_patterns, bearish_htf_fvg_patterns = self.detect_fvg_htf(
            ohlc_resampled_df
        )
        ...
        compute_crossing_bullish_fvg = lambda fvg_pattern: self.retrieve_fvg_crosses(
            fvg_pattern, ohlc_resampled_df["low"], self.htf_fill_pct
        )

        compute_crossing_bearish_fvg = lambda fvg_pattern: self.retrieve_fvg_crosses(
            fvg_pattern, ohlc_resampled_df["high"], 1 - self.htf_fill_pct
        )

        # Filter Bullish FVG patterns:
        print(
            f"{self.tag}::     Found {len(bullish_htf_fvg_patterns)} Bullish FVG patterns in the last 14 days on H4."
        )
        bullish_htf_fvg_patterns = pd.DataFrame(bullish_htf_fvg_patterns)

        bullish_htf_fvg_patterns.loc[:, "crossings"] = bullish_htf_fvg_patterns.apply(
            compute_crossing_bullish_fvg, axis=1
        )
        filt_htf_fvg_patterns = bullish_htf_fvg_patterns[
            bullish_htf_fvg_patterns["crossings"] < self.max_htf_number_crossings * 2
        ]
        ...
        print(
            f"{self.tag}::     Number of Bulish FVG patterns before crossing filtering:  {self.all_bullish_htf_fvg_patterns.shape[0]}"
        )
        print(
            f"{self.tag}::     Numver of Bulish FVG patterns after crossing filtering:   {self.filt_bullish_htf_fvg_patterns.shape[0]}"
        )

        # Filter Bearish FVG patterns:
        print(
            f"{self.tag}::     Found {len(bearish_htf_fvg_patterns)} Bearish FVG patterns in the last 14 days on H4."
        )
        bearish_htf_fvg_patterns = pd.DataFrame(bearish_htf_fvg_patterns)

        bearish_htf_fvg_patterns.loc[:, "crossings"] = bearish_htf_fvg_patterns.apply(
            compute_crossing_bearish_fvg, axis=1
        )
        filt_htf_fvg_patterns = bearish_htf_fvg_patterns[
            bearish_htf_fvg_patterns["crossings"] < self.max_htf_number_crossings * 2
        ]
        ...
        print(
            f"{self.tag}::     Number of Bearish FVG patterns before crossing filtering:  {self.all_bearish_htf_fvg_patterns.shape[0]}"
        )
        print(
            f"{self.tag}::     Numver of Bearish FVG patterns after crossing filtering:   {self.filt_bearish_htf_fvg_patterns.shape[0]}"
        )
