...
def _prep_instance_daily(df: pd.DataFrame, strategy: str, symbol: str) -> pd.DataFrame:
    ...
    work = df.copy()
    work["total_profit"] = work["profit_open"].fillna(0) + work["profit_close"].fillna(0)
...
def _color_for_value(v: float) -> dict:
    ...
    if v > 0:
        return {"bg": "rgba(0, 113, 45, 0.10)", "border": PALETTE["green"], "text": PALETTE["green"]}
    if v < 0:
        return {"bg": "rgba(185, 28, 28, 0.10)", "border": PALETTE["red"], "text": PALETTE["red"]}
    return {"bg": "rgba(15,23,42,0.04)", "border": "rgba(15,23,42,0.20)", "text": PALETTE["muted"]}
    ...
    return html.Div([
        html.Div([
            html.Div([
                ...
                html.Div([
                    html.Button("◀", id="cal_prev_month", className="flat-btn"),
                    html.Div(id="cal_month_label", style={"minWidth":"120px","textAlign":"center","fontWeight":"800"}),
                    html.Button("▶", id="cal_next_month", className="flat-btn"),
                ], className="calendar-controls"),
            ], className="calendar-controls"),

            html.Div([
                dcc.Dropdown(id="cal_strategy",
                    options=[{"label": s, "value": s} for s in strategies],
                    value=default_strategy if default_strategy in strategies else (strategies[0] if strategies else None),
                    clearable=False, className="flat-select"),
                dcc.Dropdown(id="cal_symbol",
                    options=[{"label": s, "value": s} for s in symbols],
                    value=default_symbol if default_symbol in symbols else (symbols[0] if symbols else None),
                    clearable=False, className="flat-select"),
                dcc.DatePickerSingle(
                    id="cal_anchor_date", date=this_month, display_format="MMMM YYYY",
                    className="flat-date"),
                html.Div([
                    html.Span("Legend: "),
                    html.Span(className="legend-dot", style={"background":"rgba(0,113,45,.35)"}), html.Span("Profit", style={"marginRight":"12px"}),
                    html.Span(className="legend-dot", style={"background":"rgba(185,28,28,.35)"}), html.Span("Loss"),
                ], style={"fontSize":"12px","color":"#6b7280"})
            ], className="calendar-controls"),
        ], className="calendar-toolbar"),

        html.Div([
            html.Div([html.Div(w, className="calendar-weekday") for w in ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"]],
                     className="calendar-grid", style={"gridTemplateRows":"auto"}),
            html.Div(id="cal_grid", className="calendar-grid"),
        ], className="mb-4"),

        html.Div([
            html.H4("Monthly stats", style={"margin":"0 0 10px 0", "color":"#0f172a"}),
...
def register_daily_calendar_callbacks(app, merged_deals: pd.DataFrame):
    ...
    def render_grid(anchor_date, strategy, symbol):
        ...
        # Monthly stats
        month_mask = (daily.index >= mctx.first) & (daily.index <= mctx.last) if not daily.empty else []
        month_df = daily.loc[month_mask] if not daily.empty else pd.DataFrame(columns=["pnl","n_trades"])
        if month_df.empty:
            stats = html.Div("No trades for this month.", style={"color":PALETTE["muted"]})
        else:
...
def _stat_row(label: str, value: str) -> html.Div:
    ...
    ], style={"border":"1px solid "+PALETTE["grid"], "padding":"10px", "background":"#fff"})
