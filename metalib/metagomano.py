...
class MetaGO(MetaStrategy):
    ...
    def signals(self):
        ...
        close_last = (
            close.diff().tail(4).head(3)
        )  # Extract last 4 values, then take first 3 candles
        ...
        sl = (
            close.iloc[-1] - 12 * indicators["atr"].iloc[-1]
            if long_signal
            else close.iloc[-1] + 12 * indicators["atr"].iloc[-1]
        )
        self.sl = float(sl)
        ...
        print(
            f"{self.tag}::: True Open Monthly (last 3 values): {indicators['true_open_monthly'].tail(3).to_list()}"
        )
        print(
            f"{self.tag}::: Uptrend Indicator (last 3 values): {indicators['uptrend'].tail(3).to_list()}"
        )
        print(
            f"{self.tag}::: Downtrend Indicator (last 3 values): {indicators['downtrend'].tail(3).to_list()}"
        )
        print(
            f"{self.tag}::: ATR (last 3 values): {indicators['atr'].tail(3).to_list()}"
        )
        ...
        print(
            f"    - All of last 4 returns (except last) are > 0: {close_positive_condition}"
        )
        ...
        print(
            f"    - All of last 4 returns (except last) are < 0: {close_negative_condition}"
        )
        print(f"    - Downtrend > 0: {downtrend > 0}")
        print(f"    - Close > True Open Monthly: {close.iloc[-1] > true_open_level}")
        print(
            f"    - Mask Downtrend Above Yearly: {mask_downtrend_above_yearly.iloc[-1]}"
        )
    ...
    def retrieve_indicators(self, ohlc_df):
        ohlc = ohlc_df.copy()
        ohlc_daily = ohlc.resample("1D").agg(
            {
                "open": "first",
                "high": "max",
                "low": "min",
    ...
    def get_positions_info(self):
        ...
        if filtered_positions:
            ...
            mid_price = (
                tick_info.last
                if tick_info.last
                else (tick_info.bid + tick_info.ask) / 2
            )
            num_positions = 0
