...
class MetaVolAn(MetaStrategy):
    ...
    def signals(self):
        ...
        for symbol in self.symbols:
            ...
            current_vol = returns.rolling(window=self.vol_window).std() * np.sqrt(
                self.vol_window
            )
            ...
            print(
                f"{self.tag}::: {symbol} - Current 4h Vol: {current_vol_value:.4f}, Rank: {vol_rank:.2f}"
            )
        ...
        if len(self.symbols) > 1:
            ...
            self.covariances = returns_df.cov() * (
                252 * 6
            )  # Assuming 4h bars, 6 per day
            ...
            for i, row in enumerate(signal_data):
                ...
                signal_data[i]["volatility_contribution"] = self.covariances[symbol][
                    symbol
                ]
    ...
    def check_conditions(self):
        ...
        for _, row in self.signalData.iterrows():
            ...
            if vol_rank > 0.95:
                print(
                    f"{self.tag}::: {symbol} EXTREME HIGH volatility detected (rank: {vol_rank:.2f})"
                )
                ...
                print(
                    f"{self.tag}::: {symbol} EXTREME LOW volatility detected (rank: {vol_rank:.2f})"
                )
        ...
        if self.correlations is not None and len(self.symbols) > 1:
            ...
            upper_corr = self.correlations.where(
                np.triu(np.ones(self.correlations.shape), k=1).astype(bool)
            )
            ...
            if max_corr > 0.9:
                ...
                self.send_telegram_message(
                    f"⚠️ HIGH CORRELATION detected: {max_corr:.3f}"
                )
            elif min_corr < -0.9:
                print(
                    f"{self.tag}::: HIGH NEGATIVE CORRELATION detected: {min_corr:.3f}"
                )
                self.send_telegram_message(
                    f"⚠️ HIGH NEGATIVE CORRELATION detected: {min_corr:.3f}"
                )
    ...
    def fit(self):
        ...
        if len(self.symbols) > 1:
            ...
            print(
                f"{self.tag}::: Fitted {len(self.fitted_correlations)} correlation matrices"
            )
            print(
                f"{self.tag}::: Fitted {len(self.fitted_covariances)} covariance matrices"
            )

        print(
            f"{self.tag}::: Loaded {len(self.data[self.symbols[0]])} periods for volatility analysis"
        )
        print(
